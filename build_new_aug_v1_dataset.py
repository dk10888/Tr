...
# Hard negative single/double character OCR residue fragments (Pattern 6 - Highest Priority Fix)
...
def generate_space_split_variants(text: str, n_variants: int = 2) -> list:
    variants = []
    chars = list(text)
    if len(chars) < 2:
        return []
    ...
    for _ in range(n_variants):
        ...
        for i, c in enumerate(chars):
            spaced += c
            if i < len(chars) - 1 and c != ' ' and random.random() < 0.45:
                spaced += " "
...
def build_full_expanded_dataset():
    ...
    with open(V25_CSV, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            src = row.get("source", "")
            # Filter out noisy unguided OCR aug rows
    ...
    # 1. Pattern 6 [HIGHEST PRIORITY]: Single/Double Char Residue HARD NEGATIVES
    print("  ✨ Pattern 6: Adding isolated 1-2 char residue hard negatives...")
    ...
    # 2. Pattern 2 & 7: FULL Word Space Splitting across ALL food & non-food items
    print("  ✨ Pattern 2 & 7: Generating space-split variants for ALL items...")
    for t in raw_food_texts:
        for var in generate_space_split_variants(t, n_variants=2):
            rows_out.append({"text": var, "label": "food", "source": "pattern2_word_space_split"})

    for t in raw_not_food_texts:
        if len(t) >= 3 and random.random() < 0.5:
            for var in generate_space_split_variants(t, n_variants=1):
                rows_out.append({"text": var, "label": "not_food", "source": "pattern2_not_food_space_split"})

    # 3. Pattern 4: Leading symbol noise for ALL food items (#돈까스, ~콜라)
    ...
    # 4. Pattern 1: Digit-letter confusion across ALL numeric/price lines
    ...
    # 5. Pattern 3: Adjacent item merging (5,000 merged food pairs)
    print("  ✨ Pattern 3: Generating 5,000 merged adjacent menu item pairs...")
    for _ in range(5000):
    ...
    print(f"\n🚀 Total Rows in fully expanded new_aug_v1 dataset: {len(rows_out)}")
    ...
    # Train FastText on new_aug_v1
    output_dir = os.path.join(BASE_DIR, "fastText_new_aug_v1")
    os.makedirs(output_dir, exist_ok=True)

    tmp_train = "fasttext_train_new_aug_v1.tmp"
    with open(tmp_train, "w", encoding="utf-8") as f:
        for r in rows_out:
            f.write(f"__label__{r['label']} {r['text']}\n")

    print("\n🚀 Training FastText Model on fully expanded 'korean_receipt_dataset_new_aug_v1'...")
    ...
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump({
            "model": "FastText korean_receipt_dataset_new_aug_v1 (Fully Expanded)",
    ...
    print("\n" + "="*70)
    print("🏆 FULLY EXPANDED NEW AUG V1 COMPLETED SUCCESSFULLY!")
    print("="*70)
